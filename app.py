...
with st.sidebar:
    ...
    if CLOUD_MODE:
        if not df.empty:
            ...
            badge_cls  = "badge-green" if last_date == today else "badge-yellow"
            status_txt = f"CLOUD · {last_date}"
        else:
            badge_cls, status_txt = "badge-red", "NO DATA"
        st.markdown(
            f'<span class="badge {badge_cls}">{status_txt}</span>',
            unsafe_allow_html=True,
        )
        ...
        if files_today:
            ...
            try:
                ...
                st.markdown(
                    ...
                    f'最後更新 {last_dt.strftime("%H:%M:%S")}</div>',
                    unsafe_allow_html=True,
                )
            except Exception:
                st.markdown(
    ...
    sidebar_stats(
        ...
        total_count=len(df),
        date_range_start=str(df["Date"].max()) if not df.empty else "—",
    )
    ...
    st.markdown(
        f'<div style="font-size:0.72rem;color:{TEXT_MUTED};letter-spacing:1px;'
        f'text-transform:uppercase;margin-bottom:8px;">DATE FILTER</div>',
        unsafe_allow_html=True,
    )
    if not df.empty and "Date" in df.columns:
        _dates = sorted(df["Date"].dropna().unique().tolist(), reverse=True)
