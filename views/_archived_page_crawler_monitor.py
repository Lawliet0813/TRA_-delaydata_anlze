...
def render(data_dir: str):
    ...
    if CLOUD_MODE:
        st.info(
            "雲端展示模式下此頁面不可用。這個部署只讀取 GitHub Raw 鏡像檔；"
            "實際採集來源已切換為本機 launchd。若要看執行紀錄，請查看本機 `~/Library/Logs/tra_*.log`。"
        )
        return
    ...
    # ══ Section 4：停用中的 GitHub workflow vs 本地比較 ═══════════════════
    st.markdown("<div style='margin-top:24px;'></div>", unsafe_allow_html=True)
    section_title("停用中的 GitHub workflow 與本機 launchd 比較")

    rows = [
        ("角色",            "保留中的備援 workflow",       "目前唯一自動採集來源"),
        ("排程方式",        "GitHub 伺服器排程（目前停用）",  "launchd StartInterval / StartCalendarInterval"),
        ("可用性",          "若重啟可全天候執行",            "需 Mac 開機且不可休眠"),
        ("憑證管理",        "GitHub Secrets（加密）",       ".env 明文（存本機）"),
        ("Python 版本",     "3.11（Ubuntu）",               "3.14（本機 venv）"),
        ("Raw JSON 推送",   "曾可直接 commit 到 repo",       "保留本機，節省儲存庫空間"),
        ("CSV 推送機制",    "同一儲存庫直接 commit",         "複製到 ~/tra_git_push 後再 push"),
        ("並發衝突處理",    "concurrency group 保護",        "單一 process，無衝突"),
        ("執行日誌",        "GitHub Actions 介面",           "~/Library/Logs/tra_*.log"),
        ("執行成本",        "消耗 Actions 額度",            "本機電力"),
        ("當前狀態",        "⏹ 已停用",                    "▶ 運行中（launchd loaded）"),
    ]
        ...
        <table style="width:100%;border-collapse:collapse;">
            <thead>
                <tr>
                    ...
                    <th style="{header_style}width:39%;color:{YELLOW};">GitHub workflow（停用）</th>
